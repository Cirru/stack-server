...
(defn
 ...
 (if
  ...
  (let
   [cursor (first items)]
   (if
    (depends-on? cursor new-item deps-info 0)
    (if
     (depends-on? new-item cursor deps-info 0)
     (if
      (contains? def-names (:kind (get deps-info new-item)))
 ...
 (if
  ...
  (let
   [cursor
    (first items)
    next-acc
    (deps-insert [] cursor acc deps-info)]
...
(defn
 ...
 [file-info]
 (let
  [[ns-name ns-line definitions procedure-line]
   file-info
   var-names
   (->>
    (keys definitions)
    (map
     (fn [var-name] (last (string/split var-name (re-pattern "/")))))
    (into (hash-set)))
   deps-info
   (->>
    definitions
       ...
       [path
        ...
        (->>
         ...
         (filter
          (fn
           [token]
           (and (contains? var-names token) (not= token var-name))))
         (into (hash-set)))]
       [var-name {:kind (first tree), :tokens dep-tokens}])))
    (into {}))
   self-deps-names
   (filter (fn [x] (depends-on? x x deps-info 0)) var-names)
   sorted-names
   (deps-sort [] (into [] var-names) deps-info)
   declarations
   (->>
    self-deps-names
   ...
   (into
    []
    (concat [ns-line] declarations definition-lines procedure-line))
   code
   (sepal/make-code tree)]
...
(defn
 ...
 (let
  [package
   (:package collection)
   namespace-names
   (into (hash-set) (keys (:namespaces collection)))
   namespace-names'
   (into
    (hash-set)
    (distinct
       ...
       [ns-name
        ...
        (->>
         ...
         (filter
          (fn
           [entry]
           (string/starts-with? (key entry) (str ns-name "/"))))
         (into {}))
    ...
    (filter
     (fn
      ...
      (let
       [[k v] pair]
       (if
        (= v (get @files-cache-ref k))
        (do (comment println "File remains:" k) false)
        (do
         (swap! files-cache-ref assoc k v)
         (println "File compiled:" k)
         true)))))
